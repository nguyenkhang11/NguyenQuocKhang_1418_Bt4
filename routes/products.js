...
router.get('/', function (req, res, next) {
  ...
  if (queries.page && (isNaN(page) || page <= 0)) {
    return res.status(400).send({ message: "Invalid page parameter" });
  }
  if (queries.limit && (isNaN(limit) || limit <= 0)) {
  ...
  if (maxPrice < minPrice) {
    return res.status(400).send({ message: "maxPrice cannot be less than minPrice" });
  }

  let result = data.filter(
    function (e) {
      let match = (!e.isDeleted) && e.title.includes(titleQ) &&
        e.price >= minPrice && e.price <= maxPrice;

      // Implement strict slug filtering
      if (match && queries.slug) {
        match = e.slug === queries.slug;
      }
...
router.post('/', function (req, res, next) {
  ...
  // Validation: title and price are required, price must be a number
  if (!title || price === undefined || price === null || price === "" || isNaN(parseFloat(price))) {
    return res.status(400).send({ message: "Title and price are required, and price must be a valid number." });
  }

  let newObj = {
    id: (getMaxID(data) + 1) + '',
    title: title,
    // Automatic slug generation (ignoring client provided slug)
